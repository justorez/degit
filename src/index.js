...
import { rimrafSync, rimraf } from 'sander';
import {
  DegitError,
  exec,
  fetch,
...
class Degit extends EventEmitter {
  constructor(src, opts = {}) {
    ...
    this.proxy = opts.proxy || process.env.https_proxy;

    this.repo = parse(src, opts);
    ...
    this.directiveActions = {
      clone: async (dir, dest, action) => {
        ...
        d.on('info', event => {
          console.log(
            chalk.cyan(`ℹ️ ${event.message.replace('options.', '--')}`)
          );
        });
    ...
    try {
      ...
      if (files.length > 0) {
        if (this.force) {
          this._info({
            code: 'DEST_NOT_EMPTY',
            message: `Destination directory isn't empty. Using options.force, continuing`
          });
        } else {
          throw new DegitError(
            `Destination directory isn't empty, aborting. Use options.force to override`,
            {
              code: 'DEST_NOT_EMPTY'
            }
          );
  ...
  async _cloneWithTar(dir, dest) {
    ...
    const cached = readJson(path.join(dir, 'map.json')) || {};
    ...
    if (!hash) {
      throw new DegitError(`Couldn't find commit hash for ${repo.ref}`, {
        code: 'MISSING_REF',
        ref: repo.ref
      });
    }
...
function parse(src, opts) {
  ...
  if (!match) {
    throw new DegitError(`Couldn't parse ${src}`, {
      code: 'BAD_SRC'
    });
  }
  ...
  if (!supported.has(site)) {
    throw new DegitError(
      `Degitc supports GitHub, GitLab, Sourcehut and BitBucket`,
      {
  ...
  try {
    ...
    return stdout
      ...
      .map(row => {
        ...
        if (!match)
          throw new DegitError(`Couldn't parse ${ref}`, {
            code: 'BAD_REF'
          });
    ...
    throw new DegitError(`Couldn't fetch remote ${repo.url}`, {
...
function updateCache(dir, repo, hash, cached) {
  // update access logs
  const logs = readJson(path.join(dir, 'access.json')) || {};
